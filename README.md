...
[Docs](https://travisluong.github.io/shadriz)<br>
...
The `id`, `created_at`, and `updated_at` fields are automatically generated by shadriz, and should be omitted from the command.
...
These commands work best when the column name is singular, (e.g. `category:references`), so that shadriz can reference the correct foreign key field (e.g. `category_id`).
...
```bash
npx shadriz@latest join posts posts_tags tags
```
...
```bash
npx shadriz@latest join tags posts_tags posts
```
