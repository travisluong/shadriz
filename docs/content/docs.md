...
[UPDATE](https://ui.shadcn.com/docs/react-19) for Next.js 15 + React 19. You may need to first run `npm config set legacy-peer-deps true` in order to bypass a dependency error.
...
```
npx shadts@latest scaffold category -c name:text
```

Create a `posts` schema:

```
npx shadts@latest scaffold post -c title:text category:references_combobox content:text_tiptap is_draft:boolean published_at:timestamp
```
...
```bash
npx shadts@latest init -p npm --latest --db-dialect sqlite -pk cuid2 --auth-solution authjs --auth-providers github,google,credentials --admin
```
...
The `id`, `created_at`, and `updated_at` fields are automatically generated by shadts, and should be omitted from the command.
...
```
npx shadts@latest scaffold products -c title:varchar price:decimal description:text stock_quantity:integer
```
...
The following are data types that can be used with the `scaffold` command. Most of the data types in Drizzle ORM are also supported in shad.ts.
...
shadts supports the following primary key generation strategies:
...
The strategy that you choose during the `init` process will be saved in `shadts.config.json`. This will be used for the authentication, stripe, and scaffold schemas.

## Foreign key constraints

shadts supports adding foreign key constraints using the following special data types:
...
```bash
npx shadts@latest scaffold category -c title:text
```
...
```bash
npx shadts@latest scaffold post -c category:references title:text
```

These commands work best when the column name is singular, (e.g. `category:references`), so that shadts can reference the correct foreign key field (e.g. `category_id`).
...
```bash
npx shadts@latest scaffold post -c category:references_combobox title:text
```
...
```bash
npx shadts@latest scaffold post -c category:references_select title:text
```
...
```bash
npx shadts@latest join tags posts_tags posts
```

## File uploads

shadts supports a `file` data type. This creates a text db column to store the file path along with a basic ui for uploads to the file system

Example:

```bash
npx shadts@latest scaffold media -c title:text image:file video:file
```
...
In development, shadts will put the files in `public/uploads`, so that they can be served during development. This works in development because routes are compiled without running a new build.

In production, shadts will put the files in `/var/www/uploads`. You'll have to find a way to serve these files. For example, pointing an nginx location `/uploads/` to the `/var/www/uploads` folder. Note: This won't work in serverless environments. If you're using serverless, consider using object storage like s3.
...
shadts provides a `create-user.ts` script to create test users.

shadts uses the `jwt` strategy of Auth.js. If you need `database` sessions, you will have to provide the implementation. Note: the `credentials` provider only supports the `jwt` strategy.
...
To see a list of available add-ons, run `npx shadts@latest add -h`.
...
```bash
npx shadts@latest scaffold posts -c title:text content:text_tiptap
```

## Naming conventions

shadts uses naming conventions as described in the table below. Number and case transformations will be applied to the generated code.
...
Developers will often use libraries to add capabilities such as authentication, data validations, and payments. However, setting these up can be time consuming. shadts provides an `init` command which allows you to choose from a short menu of features that you can add to your Next.js app. shadts will write all the code necessary for the selected features.

By having a simple working example, you'll save time not having to build it entirely from scratch. You can customize the generated code to fit your project requirements. Additionally, shadts will display a checklist of tasks to complete the initial configuration. The `init` command is intended to be run once at the beginning of a new project.
...
shadts provides a `scaffold` command to automate the entire process of setting up the initial "boilerplate" code. You only have to provide the table name along with the columns and data types. shadts will generate the database migration files, back end code, and front end code for the provided database schema.
...
Nostalgia for Ruby on Rails style development is one motivation that led to the creation of shad.ts. The `shadts scaffold` command was modeled after the `rails scaffold` command. With a predefined set of conventions, you'll spend less time configuring things, and more time building.
...
Many of the full stack patterns used in shad.ts are based on the official Next.js documentation. Next.js provides many conveniences out of the box, such as file system routing, server side rendering, code bundling, and more.

### shadcn/ui

shadcn/ui is the tool that copies and pastes beautifully styled components into your projects. Similarly, shad.ts generates full stack components into your Next.js project. You have full control of the code that is generated instead of the code being hidden behind an external package.
...
shad.ts favors proven, open-source solutions that let you maintain control over your data. Similarly, with shad.ts, you own not only your data but also your code.

### Zod

shad.ts uses `zod` and `drizzle-zod` for data validations. Each server action that is generated by the scaffolding tool will also contain zod validations to check for the correctness of data being submitted.

## FAQ

**What can I build with shad.ts?**

shad.ts is suitable for full stack monolithic server side rendered web applications. It is a full stack tool kit that automates away the time consuming things you need to do at the start of a new full stack Next.js project, saving you days worth of boilerplate coding.
...
Boilerplates go obsolete fast. shad.ts offers a `latest` option to install latest dependencies. This means you'll get the latest version of Drizzle ORM, shadcn/ui components, Auth.js, Stripe, TailwindCSS, Zod, and more. If you prefer a more stable version, choose the `pinned` option during initialization and you'll get the pinned versions of each top-level dependency. The pinned versions can be found in `package-shadts.json` in the shad.ts GitHub repo.
