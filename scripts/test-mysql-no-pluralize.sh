...
cd demo-mysql
shadrizz init -p pnpm --latest --db-dialect mysql -pk uuidv7 --auth-solution authjs --auth-providers github,google,postmark,nodemailer,credentials --admin --no-pluralize
...
shadrizz scaffold -a admin category -c name:varchar
shadrizz scaffold -a admin post -c category_id:references_select title:varchar likes:int published_at:timestamp content:text_tiptap
# shadrizz add stripe 
npm run generate
npm run migrate
